...
use crate::{
    ...
    piece::{self, ColouredPiece, Piece, PlayerColour},
};
...
impl ChessGame for GameState {
    ...
    fn do_move(&mut self, mov: &Self::Move) -> Self::UnMove {
        if mov.from == mov.to {
            ...
            if mov.from == 0 || mov.to == 0 || mov.from == 4 {
                // mov to 4 without moving from 4 would be taking the king
                self.meta.white_qs_castle = false
            }
            if mov.from == 7 || mov.to == 7 || mov.from == 4 {
                // mov to 4 without moving from 4 would be taking the king
                self.meta.white_ks_castle = false
            }
            if mov.from == 56 || mov.to == 56 || mov.from == 60 {
                // mov to 60 without moving from 60 would be taking the king
                self.meta.black_qs_castle = false
            }
            if mov.from == 63 || mov.to == 63 || mov.from == 60 {
                // mov to 60 without moving from 60 would be taking the king
            ...
            if self.board.pieces[fy][fx].unwrap().piece() == Piece::King {
                if fx == 4 && tx == 6 {
                    ...
                    self.board.pieces[fy][5] =
                        Some(ColouredPiece::from_parts(self.turn, Piece::Rook));
                    self.board.pieces[fy][7] = None;
                } else if fx == 4 && tx == 2 {
                    debug_assert!(fy == 0 || fy == 7);
                    self.board.pieces[fy][3] =
                        Some(ColouredPiece::from_parts(self.turn, Piece::Rook));
...
impl GameState {
    ...
    fn optionaly_add(
        ...
        if legal_pos(new_x, new_y)
            && !ColouredPiece::opt_is_col(self.board.pieces[new_y][new_x], col, false)
        {
    ...
    /// rook moves
    fn rook_moves(&self, col: PlayerColour, x: usize, y: usize, moves: &mut Vec<FesMoveDet>) {
        let mut dist = 1;
    ...
    /// bishop moves
    fn bishop_moves(&self, col: PlayerColour, x: usize, y: usize, moves: &mut Vec<FesMoveDet>) {
        let mut dist = 1;
                ...
                if let Some(piece) = self.board.pieces[y][x] {
                    ...
                    match piece.piece() {
                        Piece::Pawn => {
                            ...
                            for nx in nxs {
                                if nx < 8
                                    && (!ColouredPiece::opt_is_col(
                                        self.board.pieces[ny][nx],
                                        piece_col,
                                        true,
                                    ) || (y == ypassant
                                        && self
                                            .meta
                                            .enpasant_col
                                            .is_some_and(|col| col as usize == nx)))
                                {
                                    ...
                                    if can_prom {
                                                ...
                                                .is_some_and(|col| col as usize == nx)
                                        {
                                            ...
                                            FesMoveDet::push_take(
                                                &mut moves, from, to, take, &self.meta,
                                            );
                                        }
                                    }
                                }
    ...
    fn validate_move(&mut self, mov: &FesMoveDet) -> bool {
        ...
        self.unmove(mov);
        return !prelim_moves.iter().any(|mov| {
            if let Some(Piece::King) = mov.take {
                true
            } else {
                false
            }
